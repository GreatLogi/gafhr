...
@section('admin')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Summary Report UNMISS</h5>
                    </div>
                    <ul class="breadcrumb">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header"
                        ...
                        <img src="{{ asset('assets/images/auth/unmiss.png') }}" style="width: 5%;" class="rounded"
                            alt="...">
                        <h3 style="color:rgb(8, 141, 218);">PAST UNMISS GHANBATT DEPLOYMENT DETAILS</h3>
                        <hr>
                    </div>
                    <div class="card-body" style="padding-top: 10px">
                        <div class="row">
                            <div class="col-md-6">
                                <form action="{{ route('api-filter-unmiss') }}" method="POST" id="filter-form">
                                    @csrf
                            ...
                            <div class="col-md-3">
                                <div class="card support-bar overflow-hidden">
                                    <div class="card-body pb-0">
                                        @php
                                            $latestGhannattCount = \App\Models\GafMissionRecord::where('mission_name', 'UNMISS')
                                                ->whereIn('status', [5, 6])
                                        ...
                                        <div class="row text-center">
                                            ...
                                            <div class="col">
                                                <h4 class="m-0 text-white">
                                                    {{ $UnimissMaleCount = \App\Models\GafMissionRecord::where('gender', 'MALE')->where('mission_name', 'UNMISS')->whereIn('status', [5, 6])->count() }}
                                                </h4>
                                                <span>MALE</span>
                                            </div>
                                            <div class="col">
                                                <h4 class="m-0 text-white">
                                                    {{ $UnimissFemaleCount = \App\Models\GafMissionRecord::where('gender', 'FEMALE')->where('mission_name', 'UNMISS')->whereIn('status', [5, 6])->count() }}
                                                </h4>
                                                <span>FEMALE</span>
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="card support-bar overflow-hidden">
                                    <div class="card-body pb-0">
                                        @php
                                            $latestGhannattCount = \App\Models\GafMissionRecord::where('mission_name', 'UNMISS')->where('status', 2)->whereNotNull('ghanbatt_name')->whereDate('departure_date', '<=', now())->latest('departure_date')->count();
                                            // Check if departure date is today
                                            $UnmissMaleCount = \App\Models\GafMissionRecord::where('gender', 'MALE')->where('mission_name', 'UNMISS')->where('status', 2)->whereDate('departure_date', '<=', now())->latest('departure_date')->count();

                                            // Check if departure date is today
                                            $UnmissFemaleCount = \App\Models\GafMissionRecord::where('gender', 'FEMALE')->where('mission_name', 'UNMISS')->where('status', 2)->whereDate('departure_date', '<=', now())->latest('departure_date')->count();
                                        @endphp
                                    ...
                                    <div class="card-footer bg-success text-white">
                                        <div class="row text-center">
                                            ...
                                            <div class="col">
                                                <h4 class="m-0 text-white">{{ $UnmissMaleCount }}</h4>
                                                <span>MALE</span>
                                            </div>
                                            <div class="col">
                                                <h4 class="m-0 text-white">{{ $UnmissFemaleCount }}</h4>
                                                <span>FEMALE</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="card-body">
                        <div class="row align-items-center m-l-0">
                            ...
                            <div class="dt-responsive table-responsive">
                                <table id="unmiss-report" class="table table-striped table-bordered nowrap">
                                    <thead>
                                        <tr>
                                            <th>
                                                No.
    ...
    <script>
        $(document).ready(function() {
            var dataTable;
            dataTable = $('#unmiss-report').DataTable({
            ...
            $('#filter-form').submit(function(e) {
                e.preventDefault();
                dataTable.ajax.url("{{ route('api-filter-unmiss') }}").settings()[0].ajax.type = 'POST';
                dataTable.draw();
            });

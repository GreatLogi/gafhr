...
@section('admin')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Summary Report UNISFA GHANMED</h5>
                    </div>
                    <ul class="breadcrumb">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header"
                        ...
                        <img src="{{ asset('assets/images/auth/UNISFA GHANMED.png') }}" style="width: 5%;" class="rounded"
                            alt="...">
                        <h3 style="color:rgb(8, 141, 218);">PAST UNISFA GHANMED GHANBATT DEPLOYMENT DETAILS</h3>
                        <hr>
                    </div>
                    <div class="card-body" style="padding-top: 10px">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <form action="{{ route('api-filter-ghanmed') }}" method="POST" id="filter-form">
                                    @csrf
                            ...
                            <div class="col-md-3">
                                <div class="card support-bar overflow-hidden">
                                    <div class="card-body pb-0">
                                        @php
                                            $latestGhannattCount = \App\Models\GafMissionRecord::where('mission_name', 'UNISFA GHANMED')
                                                ->whereIn('status', [5, 6])
                                                ...
                                                ->where('departure_date', function ($query) {
                                                    $query
                                                        ...
                                                        ->where('mission_name', 'UNISFA GHANMED')
                                                        ->whereIn('status', [5, 6])
                                                        ->whereNotNull('ghanbatt_name');
                                                })
                                                ->count();
                                        ...
                                        <div class="row text-center">
                                            ...
                                            <div class="col">
                                                <h4 class="m-0 text-white">
                                                    {{ $GhanMedMaleCount = \App\Models\GafMissionRecord::where('gender', 'MALE')->where('mission_name', 'UNISFA GHANMED')->whereIn('status', [5, 6])->count() }}
                                                </h4>
                                                <span>MALE</span>
                                            </div>
                                            <div class="col">
                                                <h4 class="m-0 text-white">
                                                    {{ $GhanMedFemaleCount = \App\Models\GafMissionRecord::where('gender', 'FEMALE')->where('mission_name', 'UNISFA GHANMED')->whereIn('status', [5, 6])->count() }}
                                                </h4>
                                                <span>FEMALE</span>
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="card support-bar overflow-hidden">
                                    <div class="card-body pb-0">
                                        @php
                                            $latestGhannattCount = \App\Models\GafMissionRecord::where('mission_name', 'UNISFA GHANMED')->where('status', 2)->whereNotNull('ghanbatt_name')->whereDate('departure_date', '<=', now())->count();

                                            // Check if departure date is today
                                            $GhanMedMaleCount = \App\Models\GafMissionRecord::where('gender', 'MALE')->where('mission_name', 'UNISFA GHANMED')->where('status', 2)->whereNotNull('ghanbatt_name')->whereDate('departure_date', '<=', now())->latest('departure_date')->count();

                                            // Check if departure date is today
                                            $GhanMedFemaleCount = \App\Models\GafMissionRecord::where('gender', 'FEMALE')->where('mission_name', 'UNISFA GHANMED')->where('status', 2)->whereNotNull('ghanbatt_name')->whereDate('departure_date', '<=', now())->latest('departure_date')->count();
                                        @endphp
                                    ...
                                    <div class="card-footer bg-success text-white">
                                        <div class="row text-center">
                                            ...
                                            <div class="col">
                                                <h4 class="m-0 text-white">{{ $GhanMedMaleCount }}</h4>
                                                <span>MALE</span>
                                            </div>
                                            <div class="col">
                                                <h4 class="m-0 text-white">{{ $GhanMedFemaleCount }}</h4>
                                                <span>FEMALE</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="card-body">
                        <div class="row align-items-center m-l-0">
                            ...
                            <div class="dt-responsive table-responsive">
                                <table id="ghanmed-report" class="table table-striped table-bordered nowrap">
                                    <thead>
                                        <tr>
                                            <th>
                                                No.
    ...
    <script>
        $(document).ready(function() {
            var dataTable;
            dataTable = $('#ghanmed-report').DataTable({
                ...
                ajax: {
                    url: "{{ route('api-ghanmed-report') }}",
                    type: 'POST', // Set the initial request type to GET
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: function(d) {
                        // Add form data to the DataTable's Ajax request
            ...
            $('#filter-form').submit(function(e) {
                e.preventDefault();
                dataTable.ajax.url("{{ route('api-filter-ghanmed') }}").settings()[0].ajax.type = 'POST';
                dataTable.draw();
            });
        });
    </script>
